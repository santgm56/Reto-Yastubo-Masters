...
export default {
	...
	methods: {
		notifyError(error, fallbackMessage, fallbackCode = 'API_PLAN_REPATRIATION_COUNTRIES_MODAL_ERROR') {
		...
		async loadData() {
			...
			try {
				...
				this.notifyError(e, 'No se pudo cargar la información de países.', 'API_PLAN_REPATRIATION_COUNTRIES_LOAD_ERROR');
			} finally {
		...
		async attachZone(zone) {
			...
			try {
				...
				this.notifyError(e, 'No se pudo añadir la zona al plan.', 'API_PLAN_REPATRIATION_COUNTRIES_ATTACH_ZONE_ERROR');
			} finally {
		...
		async detachZone(zone) {
			...
			try {
				...
				this.notifyError(e, 'No se pudo quitar la zona del plan.', 'API_PLAN_REPATRIATION_COUNTRIES_DETACH_ZONE_ERROR');
			} finally {
				this.processingZoneId = null;
			}
		},
		...
		async toggleAttachCountry(country) {
			...
			try {
				...
				this.notifyError(e, 'No se pudo actualizar la asignación del país.', 'API_PLAN_REPATRIATION_COUNTRIES_TOGGLE_ATTACH_ERROR');
			} finally {
